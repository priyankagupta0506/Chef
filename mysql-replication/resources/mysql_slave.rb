...
property :bind_address, kind_of: String
...
property :replicate_do_db, kind_of: [String, Array]
...
action :create do
  ...
  mysql_config 'slave' do
    ...
    action :create
    notifies :restart, "mysql_service[mysql]", :immediately
  end
 bash 'Start replication' do
  code <<-EOH
    mysql -u root -h 127.0.0.1 -pmysql -e "CHANGE MASTER TO MASTER_HOST = 'x.x.x.x', MASTER_USER = 'repl', MASTER_PASSWORD = 'mysql', MASTER_LOG_FILE = 'mysql-bin.000001', MASTER_LOG_POS = 868;"
